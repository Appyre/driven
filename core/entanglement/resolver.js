...
module.exports = class Resolver {
  ...
  get projectPath() {
    return this._projectPath || (this._projectPath = path.join(this.appPath, '../'));
  }
  ...
  glob(name) {
    ...
    if (name.indexOf('driven') === 0) {
      ...
      nameParts.shift();
      globPath = path.join(this.frameworkPath, '/' + nameParts.join('/'));
    } else {
      globPath = path.join(this.appPath, '/' + name);
    }
  ...
  getFactoryPathInfo(name) {
    ...
    if (parts.isDriven) {
      parts.factoryPods[0] = this.frameworkPath;
    }
  ...
  generateLookupPaths(name) {
    ...
    if (name.indexOf('.') === 0 || name.indexOf('/') === 0) {
      name = path.join(this.appPath, name);
    }

    // –––––– driven
    if (name.indexOf('driven') === 0) {
      name = name.replace('driven', this.frameworkPath);
    }

    // –––––– and anything else
    return [`${name}.js`, name, path.join(this.appPath, `${name}.js`), path.join(this.appPath, name)];
  }
